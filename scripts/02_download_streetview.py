"""
Descarga imagenes de Street View Static API para un rango de CASE ID,
usando LAT/LONG del Excel. Guarda en fotos/<CASEID>_<slug>/streetview_1.jpg

Requiere GOOGLE_MAPS_API_KEY en .env (raiz del proyecto).

Uso:
    python scripts/02_download_streetview.py 1 10
    (descarga CASE ID 1 al 10 inclusive)
"""
import os
import re
import sys
...
def main():
    ...
    for row in range(2, ws.max_row + 1):
        ...
        out_path = folder / "streetview_1.jpg"
        out_path.write_bytes(img_resp.content)
        print(f"CASE ID {case_id} ({nombre}): guardado en {out_path.relative_to(ROOT)}")
